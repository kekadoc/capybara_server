...
internal suspend fun PipelineContext.parseScheduleResponse(
    ...
    verifiers = listOf(ApiKeyVerifier)
) {
    val scheduleApi = Di.get<ScheduleApi>()
    if (!Config.isSourceAvailable(source)) {
        throw HttpException(
            statusCode = HttpStatusCode.NotFound,
            message = "Schedule source not found"
        )
    }
    when (source) {
...
private suspend fun Schedule.toBoard(source: ScheduleSource): ScheduleBoard {
